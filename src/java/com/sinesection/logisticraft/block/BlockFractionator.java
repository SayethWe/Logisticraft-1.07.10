...
public class BlockFractionator extends LogisticraftTileEntityBlock {
	...
	public Item getItemDropped(int par1, Random random, int par3) {
		return Item.getItemFromBlock(ModBlocks.fractionatorIdle);
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemStack) {
		...
		if (itemStack.hasDisplayName()) {
			((TileEntityFractionator) world.getTileEntity(x, y, z)).setGuiDisplayName(itemStack.getDisplayName());
		}
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, Logisticraft.instance, LogisticraftGuiHandler.guiIdFractionator, world, x, y, z);
		}
		return true;
	}
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
		return new ItemStack(ModBlocks.fractionatorIdle);
	}
	...
	public static void updateState(boolean active, World world, int x, int y, int z) {
		...
		if (active) {
			world.setBlock(x, y, z, ModBlocks.fractionatorIdle);

		} else {
			world.setBlock(x, y, z, ModBlocks.fractionatorActive);
		}
